...
## [1.0.7] - 2026-04-21 10:53 KST

### Added
- 저장 요청이 응답 없이 오래 걸릴 때 자동으로 실패 처리하는 타임아웃 보호를 추가.
- 이미지 로딩이 Fabric 경로에서 실패할 경우 수동 이미지 로딩으로 폴백하는 안정화 경로를 추가.

### Changed
- 새로 추가한 이미지는 캔버스 중앙에 배치하고, 캔버스 크기에 맞춰 자동 축소되도록 조정.
- 저장 상태가 장시간 `saving`으로 남지 않도록 종료 조건을 강화.

### Fixed
- Firebase 저장이 멈춘 것처럼 보이던 문제를 완화.
- 이미지 추가 시 일부가 잘리거나 아예 안 보이던 문제를 개선.
